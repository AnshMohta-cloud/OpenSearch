...
public final class FilterTreeCallbacks {
    ...
    public static void releaseProvider(long contextId, int providerKey) {
        try {
            // Asserting on the release path is correct under the refcounted lifecycle: a binding
            // outlives every native handle it issued, so a release can never find it missing.
            QueryBinding binding = BINDINGS.get(contextId);
    ...
    public static void releaseCollector(long contextId, int collectorKey) {
        try {
            // See releaseProvider: refcounting guarantees the binding is still present here.
            QueryBinding binding = BINDINGS.get(contextId);
