...
/// Plain-path entry (no Lucene split): evaluate a nested-descent predicate `tree` over `array_col`,
/// returning one boolean per parent row. Used by the flat `nested_any_match` UDF to correctly evaluate a
/// DEEP (dotted-field) keyword prune peer natively on DataFusion — keeping that dual-viable peer correct on
/// both backends (Lucene pruning stays a pure optimization). Does not expose the split-internal
/// `LuceneClauseBits` type.
pub(crate) fn evaluate_nested_descent(
    array_col: &ArrayRef,
    tree: &Json,
) -> Result<datafusion::arrow::array::BooleanArray> {
    evaluate_nested_any_match(array_col, tree, None)
}
...
fn eval_bool(
    ...
    // match → the ∃ loop continues), never poisoning the whole result.
    //
    // DEEP element-grain Lucene split (Phase C): we THREAD `lucene` through the nested boundary rather than
    // dropping to `None`. This is correct because `inner_idx` (the ∃-loop variable below) is the GLOBAL
    // flattened ordinal of the inner level's values array — the exact coordinate space a deep clause's
    // `clause_bits[i]` is keyed in (the executor builds each clause's bitset at ITS OWN path's deepest grain
    // via a composed-offset childBase; see single_collector.rs). A `{"lucene":i}` node only ever appears at
    // the depth whose path is clause i, so `LuceneClauseBits::value(i, inner_idx)` consults the right bit at
    // the right grain. When bits for clause i are absent on a path (not delegated / demoted), the node falls
    // back to its own subtree (see the {"lucene"} branch above) — so threading is safe on EVERY path and
    // Lucene stays a pure optimization, never a correctness dependency, at any depth.
    ...
    match op {
        ...
        // SQL LIKE / ILIKE on a string leaf. `{"op":"LIKE"|"ILIKE","args":[field, pattern]}`. The
        // rewriter drops SQL's escape operand (default '\') — standard `%` (any run) / `_` (any one
        // char) wildcards. NULL leaf ⇒ None (3VL) inside the ∃ loop (element doesn't match, loop
        // continues), matching the comparison ops.
        "LIKE" | "ILIKE" => {
            if args.len() != 2 {
                return plan_err!("nested_any_match_expr: LIKE expects 2 args");
            }
            let left = eval_value(&args[0], struct_array, struct_fields, elem_idx)?;
            let right = eval_value(&args[1], struct_array, struct_fields, elem_idx)?;
            Ok(like_match(&left, &right, op == "ILIKE"))
        }
...
/// SQL LIKE match: `%` = any run (incl. empty), `_` = exactly one char. Returns None if either
/// operand is null/non-string (3VL). `ci` = case-insensitive (ILIKE). No escape handling — the
/// rewriter strips SQL's escape operand (patterns in this path don't use it).
fn like_match(value: &ScalarValue, pattern: &ScalarValue, ci: bool) -> Option<bool> {
    if is_null(value) || is_null(pattern) {
        return None;
    }
    let (v, p) = match (scalar_to_string(value), scalar_to_string(pattern)) {
        (Some(v), Some(p)) => (v, p),
        _ => return None,
    };
    let (v, p) = if ci { (v.to_lowercase(), p.to_lowercase()) } else { (v, p) };
    Some(like_is_match(v.as_bytes(), p.as_bytes()))
}

/// Classic two-pointer SQL-LIKE matcher with `%`/`_`, linear time via backtrack anchors.
fn like_is_match(text: &[u8], pat: &[u8]) -> bool {
...
mod tests {
    ...
    fn like_matcher_semantics() {
        assert!(like_is_match(b"black", b"b%"));
        assert!(like_is_match(b"black", b"%k"));
        assert!(like_is_match(b"black", b"%a%"));
        assert!(like_is_match(b"black", b"bl_ck"));
        assert!(like_is_match(b"black", b"%"));
        assert!(like_is_match(b"black", b"black"));
        assert!(!like_is_match(b"black", b"b_"));
        assert!(!like_is_match(b"black", b"%z"));
        assert!(!like_is_match(b"black", b"red%"));
        assert!(!like_is_match(b"", b"_"));
        assert!(like_is_match(b"", b"%"));
        assert!(like_is_match(b"aaa", b"a%a%a"));
    }

    #[test]
    fn like_op_in_nested_predicate() {
        // ∃ comment whose author LIKE 'a%' — P0.alice matches, P1.alice matches.
        let array = comments_array(&[vec![("alice", 1), ("bob", 2)], vec![("carol", 3)]]);
        let json = r#"{"op":"LIKE","args":[{"field":"author"},{"lit":"a%"}]}"#;
        let tree: Json = serde_json::from_str(json).unwrap();
        let out = evaluate_nested_any_match(&array, &tree, None).unwrap();
