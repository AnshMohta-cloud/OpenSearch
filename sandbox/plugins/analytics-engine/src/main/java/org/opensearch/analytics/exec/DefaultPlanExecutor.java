...
public class DefaultPlanExecutor extends HandledTransportAction<AnalyticsQueryRequest, AnalyticsQueryResponse>
    ...
    private volatile boolean lucenePruneOnly;

    /**
     * Whether Lucene may act as a metadata-only driver for a whole stage.
     *
     * <p>Strict prune-only must suppress it: {@code PlanAlternativeSelector} can hand Lucene an
     * ENTIRE stage (the count fast path), in which case Lucene produces the result and no amount of
     * predicate-level care matters. Suppressing metadata-driver alternatives is exactly what
     * {@code prefer_metadata_driver=false} already does, so prune-only reuses it rather than adding
     * a second knob.
     */
    private boolean metadataDriverEnabled() {
        return preferMetadataDriver && lucenePruneOnly == false;
    }
    ...
    public DefaultPlanExecutor(
        ...
        this.lucenePruneOnly = AnalyticsPlugin.LUCENE_PRUNE_ONLY.get(clusterService.getSettings());
        clusterService.getClusterSettings().addSettingsUpdateConsumer(AnalyticsPlugin.LUCENE_PRUNE_ONLY, v -> lucenePruneOnly = v);
    ...
    private void executeInternal(
        ...
        boolean metadataDriver = metadataDriverEnabled();
    ...
    private void dispatchGeneralShuffle(
        ...
        new UnifiedDispatch(qscheduler, clusterService, capabilityRegistry, metadataDriverEnabled(), sortMergeJoinMinRows).run(
            context,
            dag,
